...
namespace IPSERVER
{
	...
	class MessageQueueHandle
	{
		private:
            ...
            IPSERVER::ViewLinkHandle* m_VLhandle;
		...
		public:
			...
			//发送消息
			const int SendMsg(const IPSERVER::ST_MQ_COMSEND_MSG& msg);
            ...
            //设置串口对象
            const int SetSerialPort(IPSERVER::Serial *handle);

            //设置VLHandle IPSERVER::ViewLinkHandle*
            const int SetVLHandle(IPSERVER::ViewLinkHandle* handle);

			void GetBoxes(std::vector<bbox_t> &boxes);
			std::vector<bbox_t> m_boxes;
